<template>
  <div class="blogPagination">
    <button class="blogPageButton" @click="gotoPage(currentPage - 1)" :disabled="currentPage === 1">&lt;</button>
    
    <span v-if="visiblePages[0] > 2">
      <button class="blogPageButton" @click="gotoPage(1)">1</button>
      <span class="ellipsis" @click="gotoPage(visiblePages[0] - 1)">...</span>
    </span>
    
    <span v-for="page in visiblePages" :key="page">
      <button class="blogPageButton" @click="gotoPage(page)" :class="{ active: page === currentPage }">{{ page }}</button>
    </span>
    ...
    <button class="blogPageButton" @click="gotoPage(currentPage + 1)" :disabled="currentPage === totalPages">&gt;</button>
  </div>
</template>
...
.blogPageButton.active {
  color: rgb(255, 255, 255);  
}
.blogPageButton.active::after {
  content: ""; 
  display: block;
  width: 58px; 
  height: 55px; 
